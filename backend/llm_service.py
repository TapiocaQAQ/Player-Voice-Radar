...
# ── System Prompt ─────────────────────────────────────────────
SYSTEM_PROMPT = """你是一位資深遊戲營運分析師，專職分析手機遊戲《競技麻將2》的 Google Play 玩家評論。

你的任務是：接收一批玩家評論（JSON Array），為每一筆評論新增 `ai_analysis` 欄位，然後輸出完整的 JSON Array。

【輸出格式規定】
- 直接輸出合法的 JSON Array，不得有任何前置文字、說明、markdown 標籤（如 ```json）。
- 每個物件必須保留原始欄位：review_id、player_name、date、star_rating、review_text、thumbsUpCount。
- 每個物件新增 `ai_analysis` 欄位，結構如下：

{
  "sentiment": "negative" | "positive" | "neutral",
  "category": "工程研發" | "客服金流" | "營運企劃" | "UI/UX體驗" | "行銷推廣" | "其他",
  "risk_level": "high" | "medium" | "low",
  "is_vip_player": true | false,
  "keyword": "2~5字的核心痛點標籤，例如：閃退斷線、扣款未到帳、機率不公、廣告過多",
  "root_cause_summary": "一句話總結玩家痛點與情境，不超過20字"
}

【分類判斷準則（嚴格 6 類）】
1. 工程研發：提及閃退、斷線、登入失敗、耗電、卡頓、黑畫面、伺服器異常
2. 客服金流：提及扣款未入帳、儲值失敗、誤鎖帳號、客服無回應、申訴問題
3. 營運企劃：提及機率不公、配桌不合理、破產救濟、活動設計不當、勝率太低
4. UI/UX體驗：提及按鈕太小、教學冗長、介面跑版、操作不直覺
5. 行銷推廣：提及廣告過多、彈窗干擾、宣傳不實
6. 其他：無法歸入上述任一類

備註：若涉及多部門（如看廣告閃退），一律以「工程研發」優先。

【risk_level 判斷準則】
- high：涉及金錢損失、帳號封鎖、嚴重系統錯誤（資料遺失等）
- medium：影響遊戲體驗但無直接損失（卡頓、配對不公平等）
- low：輕微不滿或純正面評論

【is_vip_player 判斷準則】
- 若評論文本中提及「課金」、「儲值」、「花了X萬」、「花了X千」、「老玩家」、「VIP」、「花錢」、「充值」等字眼 → true
- 否則 → false
...
def process_batch(client: Groq, batch: List[Dict]) -> List[Dict]:
    ...
    for attempt in range(1, MAX_RETRIES + 2):  # +2 = 原始嘗試 + 重試次數
        try:
            ...
            # ── 風險等級強制覆寫：thumbsUpCount >= 10 或 is_vip_player → high ──
            for item in parsed_list:
                ai = item.get("ai_analysis")
                if ai is None:
                    continue
                thumbs = item.get("thumbsUpCount", 0)
                is_vip = ai.get("is_vip_player", False)
                if (isinstance(thumbs, int) and thumbs >= 10) or is_vip is True:
                    ai["risk_level"] = "high"
